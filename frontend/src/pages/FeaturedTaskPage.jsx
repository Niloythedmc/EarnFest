...
const PLAN_KEY = 'featured';
const PLAN_PRICE = 20;


// ── Reusable Task Form (used twice for Task 1 and Task 2) ────────────────────
const TaskForm = ({ index, data, onChange }) => {
  ...
  return (
    <Card style={{ padding: '16px' }}>
      <p style={{ fontWeight: '800', fontSize: '0.7rem', opacity: 0.6, marginBottom: '10px' }}>
        TASK {index} — TYPE
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px', marginBottom: '12px' }}>
        {TASK_TYPES.map((type) => (
          <button
            key={type.key}
            onClick={() => onChange({ ...data, taskType: type.key, channelCheck: null, channelUsername: '' })}
            style={{
              ...
              fontSize: '0.65rem', fontWeight: data.taskType === type.key ? '700' : '500',
            }}
          >
            {type.icon}
      ...
      <label style={{ fontSize: '0.7rem', fontWeight: '700', marginBottom: '6px', display: 'block', opacity: 0.6 }}>
      ...
      <div className="input-container" style={{ marginBottom: '10px' }}>
        <LinkIcon size={14} className="gold-text" />
        <input
          type="text"
          placeholder={data.taskType === 'bot' ? '@mybot' : data.taskType === 'channel' ? 'https://t.me/...' : 'https://...'}
      ...
      <p style={{ fontWeight: '800', fontSize: '0.65rem', opacity: 0.5, marginBottom: '6px' }}>
        IMAGE (Optional)
      </p>
      {data.imagePreview ? (
        <div style={{ position: 'relative', borderRadius: '8px', overflow: 'hidden' }}>
          <img src={data.imagePreview} alt="" style={{ width: '100%', maxHeight: '140px', objectFit: 'cover', borderRadius: '8px' }} />
          <button
        ...
        >
          <ImageIcon size={24} style={{ opacity: 0.3, marginBottom: '4px' }} />
          <p style={{ fontSize: '0.65rem', opacity: 0.4 }}>Upload image</p>
        </div>
      )}
...
const FeaturedTaskPage = () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (res.data.publishStatus === 'published') {
          toast.success('Your featured tasks have been published! 🎉');
        } else {
  ...
  const handleRecheckAndPublish = async (pid) => {
    ...
    try {
      ...
      if (res.data.success && res.data.publishStatus === 'published') {
        ...
        toast.success('Featured tasks published! 🎉');
      } else {
      ...
      <div className="main-content" style={{ paddingTop: '60px' }}>
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '0.8rem', opacity: 0.6, marginBottom: '24px' }}>
            Your featured tasks and banner are now live.
          </p>
  ...
  return (
    <div className="main-content" style={{ paddingBottom: '40px' }}>
      ...
      <h2 style={{ fontSize: '1.3rem', fontWeight: '900', marginBottom: '4px' }}>Featured Task</h2>
      <p style={{ fontSize: '0.75rem', opacity: 0.5, marginBottom: '20px' }}>2 Tasks + Banner Featuring — <img src={TON_LOGO} alt="TON" style={{ width: '14px', height: '14px', verticalAlign: 'middle', marginRight: '2px' }} />{PLAN_PRICE}</p>
        ...
        {[1, 2].map((num) => (
          <button
            ...
            style={{
              ...
              background: activeTab === num ? 'rgba(74,144,226,0.15)' : 'rgba(255,255,255,0.03)',
              border: activeTab === num ? '1.5px solid #4a90e2' : '1px solid transparent',
              color: activeTab === num ? '#4a90e2' : 'rgba(255,255,255,0.5)',
      ...
      <AnimatePresence>
        {processing && (
          ...
          >
            <Loader2 size={40} className="spin" style={{ color: '#00c896', marginBottom: '16px' }} />
...
export default FeaturedTaskPage;