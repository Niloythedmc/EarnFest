...
const ScratchCardGame = () => {
  ...
  const initCanvas = () => {
    ...
    const points = [
      ...
      { x: 30, y: 90 }, { x: 270, y: 90 },
      { x: 150, y: 20 }, { x: 150, y: 100 }
    ];
    points.forEach(pt => {
      ctx.beginPath();
    ...
    ctx.fillText('⭐ SCRATCH HERE ⭐', 150, 60);
  };
  ...
  const getMousePos = (e) => {
    ...
    // Convert CSS layout coordinate mapping to fixed 300x120 buffer resolution
    return {
      x: (xCSS / rect.width) * 300,
      y: (yCSS / rect.height) * 120
    };
  };
  ...
  const handleBuyCard = async () => {
    ...
    const res = await buyScratchCard(buyingCard.id, buyQuantity);
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'shop' ? (
              ...
              >
                ...
                <div style={{ width: '100%', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '260px', flexShrink: 0, overflow: 'hidden' }}>
                  ...
                  <motion.div
                    ...
                    animate={{ x: `calc(-${selectedInventoryTierIndex * 100}% - ${selectedInventoryTierIndex * 24}px)` }}
                  ...
                  >
                    {TIERS_KEYS.map((tierKey, index) => {
                      const tier = TIERS_DETAILS[tierKey];
                      const tierCount = user?.scratchCardsCount?.[tierKey] || 0;
                      const isCurrentActive = index === selectedInventoryTierIndex;
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {tierCount === 0 && (!isPlaying || !isCurrentActive) ? (
                              ...
                              <div style={{ textAlign: 'center', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', width: '100%', padding: '0 8%' }}>
                                ...
                                <span style={{ fontSize: '5cqw', color: '#a7f3d0', textShadow: '1px 1px 0px #000' }}>0 Cards Available</span>
                                <button
                              ...
                              <div style={{ textAlign: 'center', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', width: '100%', padding: '0 8%' }}>
                                ...
                                <span style={{
                                  ...
                                  Inventory: <strong style={{ color: '#ffd700', fontSize: '7cqw' }}>{tierCount}</strong>
                                </span>
                                <span style={{
                                  fontSize: '5cqw',
                                  letterSpacing: '0.5px',
                      ...
                      >
                        <div
                          style={{
                            ...
                            transform: isSelected ? 'rotate(-12deg) scale(1.15)' : 'rotate(-12deg)',
                            border: isSelected ? '2px solid rgba(255, 215, 0, 0.9)' : '1px solid rgba(255,255,255,0.25)',
                            boxShadow: isSelected ? '0 6px 12px rgba(255, 215, 0, 0.3)' : '0 3px 6px rgba(0,0,0,0.4)',
                            transition: 'all 0.2s',
                            flexShrink: 0
                          }}
                        />
              ...
              >
                {purchaseStatus === 'loading' ? (
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', margin: '8px 0 16px 0', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '12px', padding: '8px 16px' }}>
                      <button
                    ...
                    <p style={{ margin: '0 0 24px 0', fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', maxWidth: '280px' }}>
                      Are you sure you want to purchase <strong style={{ color: '#ffd700' }}>{buyQuantity}x</strong> {buyingCard.title} for <strong style={{ color: '#ffd700' }}>{(buyingCard.price * buyQuantity).toFixed(1)} $FEST</strong>?
                    </p>
